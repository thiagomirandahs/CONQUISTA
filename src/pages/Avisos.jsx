...
export default function Avisos() {
  ...
  if (!ehAdmin) {
    return (
      <div className="bg-surface rounded-2xl p-8 text-center shadow-soft">
        ...
        <p className="text-sm text-faint">Apenas diretoria/instrutor podem enviar avisos.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={enviar} className="bg-surface rounded-2xl p-5 shadow-soft space-y-3">
        <div>
        ...
        <div>
          <label className="block text-xs font-semibold text-muted mb-1">Detalhes (opcional)</label>
        ...
        <div>
          <label className="block text-xs font-semibold text-muted mb-1">Quem recebe</label>
          <div className="flex gap-2">
            {[['todos', '👨‍👩‍👧 Todo o clube'], ['lideranca', '⭐ Só a liderança']].map(([k, lbl]) => (
              <button type="button" key={k} onClick={() => setPara(k)}
                className={`flex-1 rounded-lg py-2 text-sm font-semibold border transition ${para === k ? 'bg-gradient-to-r from-brand to-brand2 text-white border-transparent shadow-glow' : 'bg-surface text-muted border-line'}`}>{lbl}</button>
            ))}
          </div>
        </div>
      ...
      <p className="text-center text-xs text-faint mt-3">O aviso aparece no 🔔 de quem você escolher. Se o push estiver ligado, também chega no celular.</p>
...
function PopupAviso() {
  ...
  return (
    <div className="bg-surface rounded-2xl p-5 shadow-soft mt-5 space-y-3">
      <div>
        <h3 className="font-extrabold text-ink">🔔 Popup de aviso</h3>
        <p className="text-xs text-faint">Abre na tela de quem entrar no app. Escreva o que quiser.</p>
      </div>
      ...
      <div>
        <label className="block text-xs font-semibold text-muted mb-1">Quem vê o popup</label>
        <div className="flex gap-2">
          {[['todos', '👥 Todo mundo'], ['devendo', '💰 Só quem está devendo']].map(([k, lbl]) => (
            <button type="button" key={k} onClick={() => set('alvo', k)}
              className={`flex-1 rounded-lg py-2 text-sm font-semibold border transition ${cfg.alvo === k ? 'bg-gradient-to-r from-brand to-brand2 text-white border-transparent shadow-glow' : 'bg-surface text-muted border-line'}`}>{lbl}</button>
          ))}
        </div>
      </div>
