    ...
    <div>
      ...
      {subiuNivel > 0 && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          className="mb-4 rounded-2xl p-4 text-center text-white shadow-soft"
      ...
      <div className="mt-6 bg-surface rounded-2xl p-4 shadow-soft flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="font-bold text-ink text-sm">{somOn ? '🔊' : '🔇'} Som e vibração dos jogos</div>
          <div className="text-xs text-faint mt-0.5">Efeitos ao acertar, errar e vencer</div>
        </div>
        <button
          onClick={() => setSomOn(alternarSom())}
          aria-label={somOn ? 'Desligar som e vibração' : 'Ligar som e vibração'}
          className={`shrink-0 w-12 h-7 rounded-full relative transition-colors ${somOn ? 'bg-brand' : 'bg-slate-300'}`}>
