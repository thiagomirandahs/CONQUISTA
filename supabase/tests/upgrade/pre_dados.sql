...
select (select count(*) from public.config_clube) as pre_config, (select count(*) from public.desafios) as pre_cat_desafios, (select count(*) from public.versiculos) as pre_cat_versiculos, (select count(*) from public.chat_mensagens) as pre_chat_msgs, (select count(*) from public.chat_conversas) as pre_chat_conv, (select count(*) from public.bichinhos) as pre_bichinhos, (select count(*) from public.biblia_leituras) as pre_biblia, (select count(*) from public.trilha_jogos) as pre_trilha, (select count(*) from public.recordes) as pre_recordes, (select count(*) from public.jogos_liberados) as pre_liberados, (select count(*) from public.chefao_golpes) as pre_golpes, (select count(*) from public.partidas) as pre_partidas, (select count(*) from public.jogos_trilha) as pre_catalogo, (select count(*) from public.missoes_feitas) as pre_missoes, (select count(*) from public.devocional) as pre_devocional, (select count(*) from public.duelos) as pre_duelos, (select count(*) from public.desafios_unidade) as pre_desafios, (select count(*) from public.pontos) as pre_pontos, (select coalesce(sum(pontos), 0) from public.pontos) as pre_soma_pontos,
