-- MATRIZ manifesto → API (fase 3.1), pras 6 Classes Regulares 2026: o que minha_classe() entrega a
...
-- lider_a não tem nascimento cadastrado → elegível às 6 (a elegibilidade só bloqueia com nascimento conhecido)
select t.como('lider_a'); select t.pedir_clube('clube_a');
select t.permitido('lider_a inicia as 6 classes oficiais no clube A',
  $q$select public.classe_iniciar(c.id) from public.classes c join public.curriculum_versions v on v.id = c.curriculum_version_id where v.origem = 'oficial' and v.status = 'publicado'$q$, 6);
reset role;
...
select t.eq('6 matrículas, uma por classe', (select count(*) from t.mc), 6);
...
-- ---------- a matriz ----------
select t.eq('API: 149 requisitos no total das 6 classes (nenhum sumiu)', (select count(*) from t.api), (select count(*) from t.man));
...
select t.eq('API: os 7 ALTERADO_POR_OMD das 6 Regulares vêm marcados (os outros 4 do relatório de cobertura são das avançadas, não importadas)',
  (select count(*) from t.api where status_fonte = 'ALTERADO_POR_OMD'), (select count(*) from t.man where status_fonte = 'ALTERADO_POR_OMD'));
select t.eq('...e são 7', (select count(*) from t.man where status_fonte = 'ALTERADO_POR_OMD'), 7);
...
select t.eq('API: N-de-M — os 25 grupos chegam como estrutura (n_minimo, sem_repeticao, pool, opções na ordem), iguais ao manifesto; requisitos simples sem escolha',
...
select t.eq('Origem: toda observação do manifesto chega em requisito_origem().observacao_fonte, igual',
  t.n($q$select count(*) from t.man m join public.class_requirements r on r.manifesto_id = m.req_id
        join jsonb_array_elements(t.pacote() -> 'classes') c on c ->> 'id' = m.classe_id
