#!/usr/bin/env bash
# Prova que a Edge Function `gerar-documento-pdf` (com as dependências FIXADAS em versão exata)
# resolve e empacota no edge-runtime de verdade (a mesma imagem Docker que o `supabase start` usa —
...
DIR="$ROOT/supabase/functions/gerar-documento-pdf"
...
echo "==> empacotando gerar-documento-pdf com $IMG"
if docker run --rm -v "$WDIR:/f:ro" "$IMG" bundle --entrypoint /f/index.ts --output /tmp/gerar-documento-pdf.eszip -q; then
