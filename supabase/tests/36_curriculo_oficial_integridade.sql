...
select t.eq('classes: manifesto → banco sem diferença (manifesto_id, nome, idade_minima, vigente_desde, fonte_url, publicado_em, id determinístico)',
  (select count(*) from (
    (select c ->> 'id', c ->> 'nome', (c ->> 'idade_minima')::int, (c ->> 'vigente_desde')::date, c -> 'fonte_base' ->> 'url', (c -> 'fonte_base' ->> 'publicado_em')::date,
       ...
       from jsonb_array_elements(t.pacote() -> 'classes') c
     except
...
select t.eq('classes: notas de proveniência preservadas (vigente_desde_nota, nota_publicado_em, cobertura, observacao_estrutural)',
  (select count(*) from jsonb_array_elements(t.pacote() -> 'classes') c join public.classes cl on cl.manifesto_id = c ->> 'id' and cl.curriculum_version_id = t.versao_id()
    where cl.proveniencia is distinct from jsonb_strip_nulls(jsonb_build_object('vigente_desde_nota', c -> 'vigente_desde_nota', 'nota_publicado_em', c -> 'fonte_base' -> 'nota_publicado_em',
                                                                                'cobertura', c -> 'cobertura', 'observacao_estrutural', c -> 'observacao_estrutural'))), 0);
...
create view t.m_sec as
  ...
  from jsonb_array_elements(t.pacote() -> 'classes') c, jsonb_array_elements(c -> 'secoes') s;
...
select t.eq('dependências: o manifesto não declara nenhuma → nenhuma curriculum_dependencies aponta pras classes/requisitos oficiais',
  (select count(*) from public.curriculum_dependencies dep
    where (dep.alvo_tipo = 'class' and dep.alvo_id in (select id from public.classes where curriculum_version_id = t.versao_id()))
       ...
       or (dep.depende_de_tipo = 'class' and dep.depende_de_id in (select id from public.classes where curriculum_version_id = t.versao_id()))), 0);
...
select t.eq('documentos_base: 6 páginas oficiais (uma por classe), iguais às fonte_base do manifesto',
...
select t.eq('...e continua tudo igual (149 requisitos, 6 classes, 0 divergências)',
  (select count(*) from t.b_req) * 100 + (select count(*) from public.classes where curriculum_version_id = t.versao_id()) * 10 + t.divergencias_req(), 14960);
