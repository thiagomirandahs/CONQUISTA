-- =====================================================================
-- Importador: versão NOVA do manifesto arquiva a anterior + escolha aberta sem lista.
-- Rodar DEPOIS da 20260921000041. Idempotente.
--
-- Revisão pontual de Amigo IX.1 contra a página oficial (fase 3.1): o texto oficial é
-- "Completar uma especialidade na área de Artes e habilidades manuais." — categoria ABERTA, sem
-- lista. A 2026.1 (migration 40) representava isso como UMA opção artificial entre parênteses
-- (erro de representação nosso). A correção é uma versão nova do manifesto (2026.2, migration 43,
-- gerada) — a 2026.1 já importada NÃO é editada. Pra isso o importador precisa de duas coisas:
--   1) ao importar uma versão nova do MESMO identificador, ARQUIVAR a publicada anterior
--      (explicitamente, devolvido no resultado como "arquivadas") — nunca 2 publicadas ao mesmo
--      tempo; quem já começou na anterior continua nela (member_classes aponta pra classe da
--      versão em que a pessoa andou), listagem/início novos usam só a publicada;
--   2) aceitar `escolha_n_de_m` com `escolha: {n}` e SEM `opcoes` (o cartão não lista opções — a
--      categoria está no texto oficial do requisito): vira grupo com n_minimo e 0 opções, e a
--      tela pede texto livre — a mesma estrutura que companheiro.IX.1 (sem_repeticao) já usava.
-- Corpo idêntico ao da migration 39 fora esses dois pontos.
-- =====================================================================
...
values ('2026-09-21-importador-versao-nova-e-escolha-aberta.sql')
