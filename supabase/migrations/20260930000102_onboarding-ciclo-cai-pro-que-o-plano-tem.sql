-- Bug real achado testando a decisão comercial "Licença Anual" (item 101): a migration 100 fazia o
-- ciclo cair pro 'mensal' quando o pedido não tinha preço — mas hardcoded 'mensal' como fallback
-- universal supunha que TODO plano sempre tem um preço mensal. O novo plano único ("anual") só tem
-- ciclo anual: sem ciclo pedido explicitamente, a assinatura nascia com ciclo='mensal' e
-- price_id NULO (o fallback tentava cair pro mensal, não achava, e desistia sem corrigir o ciclo).
-- Corrigido: o fallback agora usa QUALQUER preço que o plano realmente tenha (o mais barato
-- primeiro), nunca supõe 'mensal'. Também troca o plano padrão hardcoded 'essencial' (arquivado da
-- vitrine) por "o primeiro plano público do catálogo" — sem inventar chave nenhuma.
...
declare v_uid uuid := auth.uid(); v_s public.onboarding_sessions; v_etapas text[] := public._onboarding_etapas();
        ...
        v_feature text; v_email text; v_prov jsonb; v_ciclo text; v_price record; v_plano_chave text;
begin
  ...
  -- ---------------- criação do clube (+ assinatura em trial) ----------------
    ...
    if v_s.club_id is null then
      ...
      if v_s.subscription_id is null then
        ...
        v_plano_chave := nullif(p_dados ->> 'plano', '');
        v_plan := null;
        if v_plano_chave is not null then
          select * into v_plan from public.billing_plans
           where chave = v_plano_chave and publico and ativo and status = 'publicado' order by versao desc limit 1;
        end if;
        if not found then
          -- sem chave pedida (ou chave não é mais pública — plano arquivado): qualquer plano público
          -- do catálogo, nunca uma chave HARDCODED que pode não existir mais (era 'essencial').
        ...
        select * into v_price from public.billing_prices
         ...
         limit 1;
        -- sem preço pro ciclo pedido: cai pro que o PLANO realmente tem (nunca supõe 'mensal' — um
        -- plano pode só vender anual, como a Licença Anual única).
        if not found then
          select * into v_price from public.billing_prices
           where plan_id = v_plan.id and ativo
             and vigente_de <= now() and (vigente_ate is null or vigente_ate > now())
           order by ciclo limit 1;
          if found then v_ciclo := v_price.ciclo; end if;
        end if;
...
values ('2026-09-30-onboarding-ciclo-cai-pro-que-o-plano-tem.sql')
